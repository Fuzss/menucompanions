package com.fuzs.menucompanions.util;
...
public class CommandRegisterer {
    ...
    public static void handleReload(String jsonName, String translationKey, BiConsumer<String, File> serializer, Consumer<FileReader> deserializer, CommandContext<CommandSource> ctx) {
        ...
        JSONConfigUtil.load(jsonName, MenuCompanions.MODID, serializer, deserializer);
    ...
    private static IFormattableTextComponent getClickableComponent(String jsonName) {
        ...
        return new StringTextComponent(jsonName).mergeStyle(TextFormatting.UNDERLINE)
                .modifyStyle(style -> style.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE, JSONConfigUtil.getFilePath(jsonName, MenuCompanions.MODID).getAbsolutePath())));
    }

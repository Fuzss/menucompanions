...
//            new EntityMenuEntry.Builder().setType(EntityType.WITHER_SKELETON).setNbt("{CustomName:'{\"text\":\"Betsy\"}'}").setNameplate(true).build(),
            new EntityMenuEntry.Builder().setType(EntityType.BLAZE).setLeft().build(),
...
//            new EntityMenuEntry.Builder().setType(EntityType.CHICKEN).setLeft().setNbt("{Passengers:[{id:zombie,IsBaby:1}]}").build(),
            new EntityMenuEntry.Builder().setType(EntityType.STRIDER).setRight().setNbt("{Saddle:1,Passengers:[{id:zombified_piglin,HandItems:[{Count:1,id:warped_fungus_on_a_stick},{}]}]}").build()
    );
